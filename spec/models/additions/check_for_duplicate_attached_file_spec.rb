...
describe CheckForDuplicateAttachedFile do
  describe ".check_for_duplicate_attached_file" do
    it "should save an attachment if it's different from the current attachment" do
      person = FactoryGirl.create(:person, photo: Rack::Test::UploadedFile.new(Rails.root.join('spec', 'fixtures', 'image.jpg'), 'image/jpeg'))
      person.photo = Rack::Test::UploadedFile.new(Rails.root.join('spec', 'fixtures', 'image2.png'), 'image/png')
    ...
    it "should not re-save an attachment if it's the same as the current attachment" do
      person = FactoryGirl.create(:person, photo: Rack::Test::UploadedFile.new(Rails.root.join('spec', 'fixtures', 'image.jpg'), 'image/jpeg'))
      person.photo = Rack::Test::UploadedFile.new(Rails.root.join('spec', 'fixtures', 'image.jpg'), 'image/jpeg')
